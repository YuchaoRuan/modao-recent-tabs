...
- 点标签 → 切换到对应画布，并在左侧「画布」列表里**滚动定位到它、将其高亮为左栏选中态**（v1.0.18 起；若你随后手动滚动列表，插件不会再抢回滚动位置）；点 × → 关闭该标签（仅本地）
- 右侧「画布 N」→ 点击展开画布列表下拉，点任意项切换
- 右侧「关闭其他」→ 只保留当前画布标签，关闭其余
- 右侧「图钉」→ 切换固定显示 / 浮动显示（浮动时标签栏自动隐藏，鼠标移到**窗口最顶部 4px** 滑出，离开自动收起；v1.0.16 起不再注入热区，操作墨刀工具栏不会误弹、也不会被拦截）
- 标签栏固定显示在墨刀**工具栏上方**（窗口最顶部，固定模式会为墨刀工具栏让位下沉；v1.0.15 起不再提供上方/下方切换）
...
- **点标签提示「未找到画布」**：Console 会随提示一并打印**结构化诊断**
  （`[modao-recent-tabs] 定位失败诊断: {...}`，含版本、面板数量、同 cid 候选行及归属/是否可见、
  折叠开关数、搜索框是否存在、历史 id）。把这段 JSON 一起发出来即可定位，不必再猜 DOM。
- 真机可随时在 Console 核对当前加载版本：`document.getElementById('md-recent-tabs-root').dataset.mdVersion`。

## 改动纪律（防止同一个 BUG 反复出现）
本仓库的每次缺陷修复都登记在 **[`CHANGELOG.md`](CHANGELOG.md)**：现象（用户原话）/ 根因 /
修复 / 锁定用例 / 真机复验状态。改代码前先读它的「已修复缺陷索引」，改完必须新增用例并登记。

发版前跑一次回归门禁：

```bash
python scripts/regress.py          # 一致性校验 + 全量回归 + A/B 对照，rc=0 才允许发版
```

> 门禁会校验：根目录与 `desktop/` 的核心副本逐字节一致、版本三处一致
> （`VERSION` / `manifest.json` / 核心 `MD_VERSION`）、全部用例通过，
> 并把**上一发版的核心**抽出来跑定位回归，确认缺陷确实是新版本引入、且已被修好。
